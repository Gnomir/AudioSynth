...
# library + tests
cd harmonic_core
cargo test                                    # 67 (62 unit + 5 integration); + `-- --ignored` drift test
cargo build --no-default-features --release    # the real no_std build
bash scripts/cross-verify.sh                   # 67/67 bit-identical on ARM (Docker + QEMU)
...
67 tests pass (62 unit + 5 integration), plus a `#[ignore]` long-run drift test; `clippy` clean on `std`, `no_std` and
