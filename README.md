...
# library + tests
cd harmonic_core
cargo test                                    # 68 (63 unit + 5 integration); + `-- --ignored` drift test
cargo build --no-default-features --release    # the real no_std build
bash scripts/cross-verify.sh                   # 68/68 bit-identical on ARM (Docker + QEMU)
...
68 tests pass (63 unit + 5 integration), plus a `#[ignore]` long-run drift test; `clippy` clean on `std`, `no_std` and
