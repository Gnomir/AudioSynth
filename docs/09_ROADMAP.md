...
**Поточний стан:** core stable, 65/65 тестів green (+1 `#[ignore]` дрейф),
...
- **Крок 2 ✅ (RFC-15).** `scripts/cross-verify.{sh,ps1}` (Docker + QEMU)
  проганяє **весь набір 65/65** на `aarch64-unknown-linux-gnu` та
...
1. ~~**Б3** (крос-компіляція + bit-exact на ARM)~~ — ✅ виконано (RFC-15:
   4 таргети чисто; 65/65 біт-у-біт на `aarch64` + `armv7-hf` під QEMU).
