...
**Поточний стан:** core stable, 63/63 тестів green (+1 `#[ignore]` дрейф),
...
- **Крок 2 ✅ (RFC-15).** `scripts/cross-verify.{sh,ps1}` (Docker + QEMU)
  проганяє **весь набір 63/63** на `aarch64-unknown-linux-gnu` та
...
1. ~~**Б3** (крос-компіляція + bit-exact на ARM)~~ — ✅ виконано (RFC-15:
   4 таргети чисто; 63/63 біт-у-біт на `aarch64` + `armv7-hf` під QEMU).
