...
(плюс друге закрите ядро `Σ(aᵏ−bᵏ)cos` — резонансний горб), з явним,
вимірюваним бюджетом вартості та смуги, і мікротюнінгом фундаменталів на
довільну шкалу (Scala `.scl` / `.kbm`).
...
> аналіз, план комерціалізації та стан проєкту в одному документі (об'єднує
> шість опублікованих артефактів). Нумеровані `01`–`12` нижче — першоджерела.
