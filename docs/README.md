...
| [20_REPEATED_INDEPENDENT_AUDIT_REPORT.md](20_REPEATED_INDEPENDENT_AUDIT_REPORT.md) | Повторний аудит: 1/3 пункти підтверджено й виправлено (застаріле формулювання `initialize→false` у `05`/`07`), 2/3 — спростовано перевіркою коду (історичні дельти розміру `Voice` в `04`/`09` — коректний запис, не чіпати; геом-кеш для Saw/Triangle вже гейтований на `Waveform::Geometric`, оптимізація не потрібна) |
| — (без окремого RFC) | Самостійний аудит перед встановленням DAW: реальна прогалина знайдена — плагін не мав обробки CC#64 (sustain pedal), хоча `11_DAW_CHECKLIST.md` §5.1 вже передбачав тест з педаллю. Реалізовано в `harmonic_synth::process` (відкладений `NoteOff` на час утримання педалі); власна ж перша реалізація внесла паніку `index out of bounds` на CLAP wildcard-ноті (`key=-1 as u8 = 255`), впіймано `clap-validator`'ом (`process-note-wildcard`), виправлено bounds-checked індексуванням. `cargo xtask validate`: pluginval strictness 8 + clap-validator 35/35 — обидва чисто |
| [21_AUDIT_PLAN_REMAINING_SURFACE.md](21_AUDIT_PLAN_REMAINING_SURFACE.md) | План аудит-раундів на поверхню, яку 8 попередніх раундів не покрили. Фази 1–4 виконано: C-ABI↔заголовок (0 розбіжностей), GUI/analyzer потоки (чисто), Cargo/xtask/vendor-патч — **знайдено й виправлено, що зібраний VST3/CLAP-бінарник фактично компілювався з `panic=unwind`, не `panic=abort`** (Cargo-профілі не успадковуються від path-залежності через межу workspace; nih-plug не має `catch_unwind` на `extern "C"` межі — розкрутка була б UB), плюс фіктивний прапорець `--pluginval PATH` у `xtask` та placeholder-URL у `Cargo.toml`; host-lifecycle/persist — `reset()` підтверджено гарантованим на всіх 3 шляхах (activate/start_processing/state-load), sustain-поля коректно поза `Params`, новий ізольований DAW-тест сустейну (`11 §5.1`) відокремлено від тесту voice-stealing; Фаза 5 (свіжі очі на HQ Bus + sustain, виконана на прямий запит попри рекомендацію плану почекати) — 5 адверсаріальних трас (sustain×voice-stealing, retrigger-під-педаллю, FIR-зсув, envelope/cutoff 1×-частота, `set_sample_rate`-реконструкт), нового бага не знайдено, код не змінено. Фаза 6 — жива DAW-перевірка, за користувачем |
