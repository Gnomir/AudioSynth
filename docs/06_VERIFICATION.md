...
| `soft_clip_joins_the_clamp_smoothly` | `poly::soft_clip` — незалежна текстова копія `character::tanh_pade` — має ту саму C²-гладкість на клампі `±3`, ту саму відсутність перельоту; регресія на випадок, якщо копії розійдуться (аудит незалежного аудитора) |
| `clamps_reject_nan_instead_of_latching_it` | `NaN`-гейн + `NaN`-velocity на `PolySynth` → скінченний вихід на 4800 семплах, не NaN назавжди (аудит незалежного аудитора: `NaN < x`/`NaN > x` завжди `false`, тож голий `if`-кламп пропускав `NaN` без змін) |
| `hq_bus_master_clip_stays_under_75db_alias_floor` | RFC-16 DoD: HQ-шина, майстер вбитий `soft_clip`-ом (~6 дБ перевантаження); енергія на НЕ-гармонічних пробних частотах `≤ −75` дБ від фундаменталу (виміряно `−94.6` дБ). Захват 2²⁰ семплів без вікна — інакше витік бічних пелюстків сильної гармоніки маскується під «аліасинг» |
...
### Unified HQ Bus (RFC-16, `docs/15_TECHNICAL_SPEC_HQ_BUS.md`)

Кожен голос у HQ-режимі тепер віддає недецимовану пару `2×`-семплів
...
Kaiser-вікно, ціль `~85` дБ). Стандалон `Voice`/C-ABI (`harmonic_voice_
set_hq` + `harmonic_voice_process`) лишається на старому, незалежному
по-голосному шляху (`Character::process_hq`, свій 13-тапний дециматор) —
жодних змін, жодної регресії публічного API.

| Метрика | Значення | Метод |
|---|---|---|
| Аліасинг майстра (DoD, `+6` дБ вхід) | **`< −75` дБ (виміряно `−94.6` дБ)** | `poly::hq_bus_master_clip_stays_under_75db_alias_floor`, 2²⁰-семпловий захват, непарні гармоніки очікувані, позагармонічні — ні |
| Стопбенд майстер-дециматора | `−80` дБ досягається на `1.166×` вихідного Найквіста (не одразу за краєм — жоден 27-тапний фільтр цього не може; 65 тапів обрано саме тому, що майстер-рівень CPU не залежить від кількості голосів, тож довший фільтр практично безкоштовний) | пряме обчислення відгуку дизайн-скриптом (не в репо) |
| Латентність | `32` семпли на `2×` = **`16` семплів на `1×`, точно** (замінює `Voice::HQ_LATENCY=3`, яка лишається чинною лише для стандалон-шляху) | `HqBusDecimator::LATENCY`, `PolySynth::HQ_LATENCY` |
| **CPU при HQ, 12 голосів** (реалістична щільна партія; фільтр Low @6kHz, drive+fold) | `+0.8 … +2.5 %` (шум вимірювання, не 50%) | `examples/bench_hq_bus.rs` |
| **CPU при HQ, 16 голосів** | `+2.4 … +2.7 %` | те саме |
| **CPU при HQ, 24 голоси** (макс. поліфонія плагіна) | `+0.5 … +1.4 %` (по суті нуль, не заявлені RFC `40–50 %`) | те саме |

**DoD «40–50 % CPU» не виконано на жодній реалістичній поліфонії (12/16/24
голоси), виміряно, а не здогадно:** по-голосний дециматор (13 тапів, 4
унікальних коефіцієнти після half-band згортання) ніколи не був домінантною
вартістю HQ-шляху — осцилятор і 5 стадій `Character`, пораховані на `2×` для
кожного голосу, коштують на порядки більше, а це навантаження не залежить
від кількості голосів, лише масштабується лінійно з нею (тому ефект майже
однаковий на 12, 16 і 24). Об'єднання `N` дециматорів в один справді зменшує
саме дециматорну роботу (`N×~7` флопс → `2×~33` флопс), але нова архітектура
**додає другий прохід `Svf`** на голос (раніше фільтр рахувався один раз, на
`1×`, після децимації; тепер — двічі, на `2×`, до децимації) — це майже
точно компенсує виграш на будь-якій кількості голосів. Чистий ефект —
статистичний нуль (`±3 %` шуму вимірювання), не 50%. Архітектурна
мета (один дециматор замість каскаду `voice-decimate → master-upsample →
master-decimate`, якого вимагав би наївний RFC-14) все одно досягнута, і
аліасинг-DoD перевиконано (`−94.6` проти `−75` дБ) — але заявлений CPU-виграш
...
## 6-bis. Платформна бітова ідентичність — виконано (RFC-15)
