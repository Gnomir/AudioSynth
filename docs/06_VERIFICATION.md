...
| **CPU при HQ (24 голоси, фільтр Low @6kHz, drive+fold)** | **`−0.2 %`** (по суті нуль, не заявлені RFC `40–50 %`) | `examples/bench_hq_bus.rs`: старий по-голосний шлях (24 незалежні `Voice` зі своїм 13-тапним дециматором) vs новий unified bus |

**DoD «40–50 % CPU» не виконано, і ось чому, виміряно, а не здогадно:**
по-голосний дециматор (13 тапів, 4 унікальних коефіцієнти після half-band
згортання) ніколи не був домінантною вартістю HQ-шляху — осцилятор і 5 стадій
`Character`, пораховані на `2×` для кожного з 24 голосів, коштують на
порядки більше. Об'єднання `N` дециматорів в один справді зменшує саме
дециматорну роботу (`24×~7` флопс → `2×~33` флопс), але нова архітектура
...
точно компенсує виграш. Чистий ефект — статистичний нуль, не 50%. Архітектурна
