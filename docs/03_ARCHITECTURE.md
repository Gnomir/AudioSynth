...
|---|---|---|
| Розробка / тести | `cargo test` | `std` (дефолт), 65 тести (60 юніт + 5 інтеграційних) |
| Bit-exact на ARM | `harmonic_core/scripts/cross-verify.sh` | Docker + QEMU: `aarch64` + `armv7-hf`, 65/65, хеш = x86-64 |
