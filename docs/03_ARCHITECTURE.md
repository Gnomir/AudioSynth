...
|---|---|---|
| Розробка / тести | `cargo test` | `std` (дефолт), 62 тести (57 юніт + 5 інтеграційних) |
| Bit-exact на ARM | `harmonic_core/scripts/cross-verify.sh` | Docker + QEMU: `aarch64` + `armv7-hf`, 62/62, хеш = x86-64 |
