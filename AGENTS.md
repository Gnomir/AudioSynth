...
```
cd harmonic_core
cargo test                                     # all 62 tests (57 unit + tests/spectrum.rs + tests/cross_platform_bit_exact.rs)
bash scripts/cross-verify.sh                    # 62/62 bit-identical on aarch64 + armv7-hf (Docker + QEMU)
