...
```
cd harmonic_core
cargo test                                     # all 68 tests (63 unit + tests/spectrum.rs + tests/cross_platform_bit_exact.rs)
bash scripts/cross-verify.sh                    # 68/68 bit-identical on aarch64 + armv7-hf (Docker + QEMU)
