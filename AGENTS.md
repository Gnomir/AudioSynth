...
```
cd harmonic_core
cargo test                                     # all 67 tests (62 unit + tests/spectrum.rs + tests/cross_platform_bit_exact.rs)
bash scripts/cross-verify.sh                    # 67/67 bit-identical on aarch64 + armv7-hf (Docker + QEMU)
